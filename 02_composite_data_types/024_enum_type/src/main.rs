...
fn main() {
    ...
    // ======================================================
    println!("\n========== РЕЗЮМЕ ==========");
    println!("  enum — тип с фиксированным набором вариантов");
    println!("  Варианты могут нести данные: Cash(u32), Card {{ amount, .. }}");
    println!("  Простым вариантам можно задать дискриминанты: Ok = 200");
    println!("  match по enum исчерпывающий — компилятор проверит все варианты");
    println!("  impl даёт enum'у методы: payment.total_amount()");
    println!("  Option<T> и Result<T, E> — это тоже enum из стандартной библиотеки");
