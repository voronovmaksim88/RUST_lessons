...
//
// 1. ЧТО ТАКОЕ Vec<T>?
//    Vec (читается "век", сокращение от vector) — это
//    ДИНАМИЧЕСКИЙ МАССИВ, предоставляемый стандартной
//    библиотекой Rust.
//
//    Простыми словами: Vec — это "умный контейнер",
//    который хранит упорядоченную последовательность
//    элементов ОДНОГО ТИПА T, и который может менять
//    свой размер во время выполнения программы
//    (расти или уменьшаться).
//
//    Внутри Vec — это тонкая обёртка (struct) над
//    тремя вещами:
//      • указатель на данные в куче (heap)
//      • длина (len) — сколько элементов сейчас
//      • capacity — сколько элементов помещается
//        до следующей реаллокации
//
//    Все элементы Vec лежат в памяти ПОДРЯД (непрерывно),
//    как и в обычном массиве [T; N]. Именно поэтому
//    доступ по индексу работает за O(1) — как в массиве.
//
// 2. ДЛЯ ЧЕГО НУЖЕН Vec?
//    - Хранение списка значений, чей размер заранее
//      неизвестен (список пользователей, задач, товаров...)
//    - Ситуации, когда элементы добавляются и удаляются
//      по ходу работы программы
//    - Сборка данных из итераторов (collect)
//    - Передача и возврат коллекций из функций
//    - Когда нужны удобные методы: sort, filter, map,
//      push, pop, contains, len и т.д.
//
// 3. КАКИЕ БЫВАЮТ "КОЛЛЕГИ" Vec?
//    В стандартной библиотеке Rust есть несколько
//    похожих коллекций, и важно понимать, чем они
//    отличаются друг от друга:
//
//    а) [T; N]    — массив ФИКСИРОВАННОГО размера.
//                    Размер известен на этапе компиляции.
//                    Данные лежат на стеке. Быстрее Vec,
//                    но изменить размер нельзя.
//
//    б) Vec<T>    — динамический массив (наш урок).
//                    Размер меняется в runtime.
//                    Данные лежат в куче. Самый
//                    универсальный выбор "по умолчанию".
//
//    в) &[T]      — срез (slice). Это "вид" на
//                    непрерывную последовательность
//                    (часть Vec, массива или другого среза).
//                    Не владеет данными, только заимствует.
//
//    г) VecDeque<T> — двусторонняя очередь (из VecDeque).
//                    Умеет O(1) push/pop с обоих концов.
//                    Данные лежат в куче, но НЕ обязательно
//                    подряд (есть внутреннее кольцо).
//
//    д) LinkedList<T> — двусвязный список. O(1) вставка
//                    в середину, но плохая локальность
//                    данных (cache-unfriendly). В Rust
//                    почти никогда не нужен.
//
//    е) BinaryHeap<T> — бинарная куча (приоритетная
//                    очередь). Данные достаются
//                    в порядке "максимум/минимум".
//
//    💡 Vec — это "рабочая лошадка". В 90% случаев,
//    когда нужен список, берите именно Vec.
//
// 4. КЛЮЧЕВЫЕ ОСОБЕННОСТИ Vec<T>:
//    • Динамический размер: можно push/pop без
//      фиксированной границы
//    • Элементы ОДНОГО типа T (гомогенная коллекция)
//    • Память выделяется в куче (heap)
//    • Элементы лежат подряд → кэш-дружелюбный доступ
//    • Безопасный: проверяет границы при индексации
//      (или через get/Option)
//    • Управление памятью автоматическое — вызывать
//      free/delete вручную НЕ нужно (RAII + Drop)
//
// 5. КОГДА ВЫБИРАТЬ Vec, А КОГДА — МАССИВ?
//    Массив [T; N]:
//      - размер точно известен и не меняется
//      - важна максимальная скорость доступа
//      - данные маленькие и помещаются на стеке
//    Vec<T>:
//      - размер заранее неизвестен
//      - нужно добавлять/удалять элементы
//      - размер коллекции может меняться
//      - размер большой (не влезает в стек)
//
// 6. КАК СОЗДАЁТСЯ Vec? (кратко, до примеров)
//    а) Vec::new()              — пустой вектор
//    б) vec![a, b, c]           — макрос с начальными
//                                  значениями
//    в) vec![value; n]          — n копий одного значения
//    г) Vec::with_capacity(n)   — пустой, но с зарезерви-
//                                  рованной памятью под n
//                                  элементов (быстрее, если
//                                  знаете будущий размер)
//    д) iter.collect::<Vec<_>>()— собрать из итератора
//
// ============================================================
