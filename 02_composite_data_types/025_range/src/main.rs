...
//
// 1. ЧТО ТАКОЕ RANGE?
//    Range (диапазон) — это встроенный тип Rust, который
//    представляет последовательность чисел или символов
//    от начального значения до конечного.
//
//    Range'и НЕ хранят все значения в памяти, а работают
//    лениво (lazy) — каждое следующее значение вычисляется
//    на лету при итерации. Поэтому диапазон 1..1_000_000
//    занимает столько же памяти, сколько и 1..2.
//
// 2. ДЛЯ ЧЕГО НУЖНЫ RANGE'И?
//    - Циклы for (основной способ «пройти N раз»)
//    - Срезы массивов/векторов/строк (&data[start..end])
//    - Итераторы и функциональные цепочки (map/filter/sum)
//    - Сопоставление в match-паттернах (0..=9 => ...)
//    - Ограничение бесконечных итераторов через take()
//
// 3. КАКИЕ БЫВАЮТ RANGE'И?
//    а) Range           start..end   — конец НЕ включается
//    б) RangeInclusive  start..=end  — конец включается
//    в) RangeFrom       start..      — от start до бесконечности
//    г) RangeTo         ..end        — от начала до end (не вкл.)
//    д) RangeToInclusive ..=end      — от начала до end (вкл.)
//    е) RangeFull       ..           — весь диапазон целиком
//
// ============================================================
...
fn main() {
    ...
    // ------------------------------------------------------------
    println!("\n=== РЕЗЮМЕ ===");
    println!("  a..b — конец не входит;  a..=b — конец входит");
    println!("  a.. / ..b / ..=b / .. — открытые формы (часто для срезов)");
    println!("  Диапазоны ленивы: значения вычисляются по ходу итерации");
    println!("  Применение: for, срезы &arr[1..5], match-паттерны, итераторы");
    println!("  Адаптеры: rev(), step_by(n), take(n) + map/filter/sum/fold");
    println!("  Работают с целыми числами и char ('a'..='z')");

    println!("\n✅ Урок по диапазонам (range) завершён!");
}
