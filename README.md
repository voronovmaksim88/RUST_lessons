...
Почти каждый урок содержит подробную **документацию в коде** — комментарии с описанием синтаксиса, примеров и сравнений. Rust умеет собирать такие комментарии в HTML-документацию:

```bash
cd 01_basics/015_anonymous_function
cargo doc --open
```

- `cargo doc` — собирает документацию из `///` и `//!`-комментариев для текущего проекта и всех его зависимостей.
- `--open` — сразу открывает собранную документацию в браузере.
- Если забыли `--open`, откройте файл `target/doc/<имя_проекта>/index.html` вручную.

Документация особенно полезна в этом курсе, потому что каждый модуль (`basic_syntax`, `capture`, `iterators`, `traits`, `patterns`) содержит развёрнутые пояснения и таблицы.
