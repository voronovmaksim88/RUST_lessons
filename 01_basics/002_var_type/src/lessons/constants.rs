...
//! ## `const`
//!
//! - Встраиваются (inline) в места использования.
//! - Не имеют адреса в памяти.
//! - Имена: `ЗАГЛАВНЫМИ_БУКВАМИ`.
//! - Должны иметь явный тип и известное на этапе компиляции значение.
//!
//! ## `static`
//!
//! - Имеют фиксированный адрес в памяти.
//! - Одна копия на всю программу.
//! - Можно сделать изменяемой (`static mut`), но это требует `unsafe`.
//!
//! | Характеристика | `const` | `static` |
//! |---------------|---------|----------|
//! | Адрес в памяти | Нет | Да |
//! | Встраивание | Да (копируется в каждое использование) | Нет (одна копия) |
//! | `mut` | ❌ | ✅ (с unsafe) |
//!
//! Для большинства случаев предпочитайте `const`.
