// ============================================================
// Урок . arithmetic operations
// ============================================================

fn main() {
    // ============================================================
    // Урок 007. БАЗОВЫЕ АРИФМЕТИЧЕСКИЕ ОПЕРАЦИИ В RUST
    // ============================================================
    //
    ...
    //
    // Для положительных чисел результат совпадает с математическим mod:
    // 29 = 4 * 6 + 5, значит остаток 5.
    //
    // Но для отрицательных Rust % сохраняет знак делимого, а не всегда даёт
    // неотрицательный результат как математический mod:
    // -7 % 3 == -1  (в математике mod дал бы 2)
    let remainder = x % y;
    ...
    // wrapping_add выполняет сложение с обрезанием старших битов (wrap around)
    ...
    // as u8 обрезает все биты кроме младших 8. По значению это совпадает
    // с математическим (евклидовым) модулем: x as u8 == x mod 256, даже для
    // отрицательных (-300 as u8 == 212). А вот оператор % в Rust ведёт себя
    // иначе: он сохраняет знак делимого (-300 % 256 == -44).
    let small_number: u8 = big_number as u8; // 300 = 0b1_0010_1100 → младшие 8 бит = 0b0010_1100 = 44
    println!(
        "300 (i32) as u8 = {} (обрезание младших 8 бит)",
        small_number
    );
    ...
    // ------------------------------------------------------------
    println!("\n=== РЕЗЮМЕ ===");
    println!("  Операторы: +  -  *  /  %   и формы с присваиванием: +=  -=  *=  /=  %=");
    println!("  Целочисленное деление отбрасывает дробную часть: 29 / 6 = 4");
    println!("  % сохраняет знак делимого: -7 % 3 == -1");
    println!("  Приоритет: унарный минус > * / % > + -  (скобки задают порядок явно)");
    println!("  Переполнение: debug — паника; безопасно: wrapping_add / checked_add");
    println!("  Разные числовые типы не смешиваются — нужно явное `as`");

    println!("\n✅ Урок по арифметическим операциям завершён!");
}
